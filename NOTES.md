...
My inital take is that the paths between surfaces don't connect well. The sections below try to be specific about where the path breaks.
...
- **No REPL.** Local prompt iteration is "edit script, re-run, look at output". `npx @ai-sdk/gateway repl` with history, model switching, and visible metadata would be a fundamentally faster loop. a CLI counterpart to teh playground targeting a different funnel stage.
...
Some of the the gateway's stronger features (`getSpendReport`, `tags`, `user` attribution, BYOK separation) live at this stage. They're not super during stages 1 through 4, which means a developer who doesn't reach stage 5 never finds out they exist. That seems worth noticing, because the audience that needs the gateway most (multi-tenant SaaS that exposes AI features to its own customers) is also the audience that converts on stage-5 features, not stage-1 features.
